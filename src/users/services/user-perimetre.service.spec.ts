...
describe('UserPerimetreService', () => {
  ...
  // ─── Lot 7.1 — Résolution CR accessibles ─────────────────────────

  describe('resoudreCrAccessibles', () => {
    /**
     * Reset complet des CR + insertion de la fixture commune aux 4
     * tests : CR_DIRECT (sans struct), CR_IN_SET_1, CR_IN_SET_2 (sans
     * struct), CR_STRUCT_1 et CR_STRUCT_2 (rattachés à CIV pour tester
     * l'expansion STRUCTURE → ses CR via resoudreCrAccessibles).
     *
     * Le seed initial du fichier crée BR_CIV / BR_BFA sans
     * fk_structure (table minimale créée par `createDataSource`), mais
     * le schéma de cette table-là inclut `fk_structure` depuis Lot 7.1.
     */
    async function resetEtSeedCrs(): Promise<
      Record<
        | 'CR_DIRECT'
        | 'CR_IN_SET_1'
        | 'CR_IN_SET_2'
    ...
    > {
      ...
      await ds.query(
        `INSERT INTO dim_centre_responsabilite (code_cr, fk_structure)
         VALUES ('CR_DIRECT', NULL),
                ('CR_IN_SET_1', NULL),
                ('CR_IN_SET_2', NULL),
                ('CR_STRUCT_1', $1),
                ('CR_STRUCT_2', $1)`,
      ...
      for (const r of rows) map[r.code_cr] = String(r.id);
      return map as never;
    }

    it('union correcte CR + CR_SET + STRUCTURE (avec expansion struct → CRs)', async () => {
      const get = await resetEtSeedCrs();

      // Insérer 3 entrées user_perimetres : CR direct + CR_SET + STRUCTURE.
      await ds.query(
        `INSERT INTO user_perimetres
         ...
         VALUES
           ...
           ($1, 'CR_SET',    NULL, ARRAY[$3, $4]::bigint[],                 'AFFECTATION', CURRENT_DATE, true, 'system'),
           ($1, 'STRUCTURE', $5,   NULL,                                    'AFFECTATION', CURRENT_DATE, true, 'system')`,
        [
          ids.userId,
          get.CR_DIRECT,
          get.CR_IN_SET_1,
    ...
    it("déduplication d'un CR présent dans plusieurs entrées", async () => {
      const get = await resetEtSeedCrs();

      // CR_DIRECT est référencé en CR direct ET dans un CR_SET avec un
      // autre CR — il ne doit apparaître qu'une fois dans le résultat.
      await ds.query(
        `INSERT INTO user_perimetres
           (fk_user, cible_type, cible_id, cible_cr_ids, origine, date_debut, actif, utilisateur_creation)
         VALUES
      ...
      const result = await service.resoudreCrAccessibles(ids.userId);
      expect(result.sort()).toEqual([get.CR_DIRECT, get.CR_IN_SET_1].sort());
      expect(result).toHaveLength(2);
    });

    it('périmètre vide → tableau vide', async () => {
      await resetEtSeedCrs();
      const result = await service.resoudreCrAccessibles(ids.userId);
      expect(result).toEqual([]);
    });

    it('lignes actif=false ignorées', async () => {
      const get = await resetEtSeedCrs();
      ...
      await ds.query(
        `INSERT INTO user_perimetres
         ...
         VALUES
           ($1, 'CR', $2, 'AFFECTATION', CURRENT_DATE, true,  'system'),
           ($1, 'CR', $3, 'AFFECTATION', CURRENT_DATE, false, 'system')`,
        [ids.userId, get.CR_DIRECT, get.CR_IN_SET_1],
      );
