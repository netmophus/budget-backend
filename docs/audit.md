...
| `IMPORT_BUDGET` | Saisie en lot via `POST /api/v1/budget/grille` (Lot 3.3). Une **seule** entrée `audit_log` par appel (pas une par cellule), avec rapport agrégé. | success ou failure | `idCible = "version=X/scenario=Y/cr=Z"`, `payloadApres = { totalCellules, inserees, modifiees, supprimees, ignorees, erreursCount }`, `commentaire` mentionne le nombre de lignes traitées, `dureeMs` consigné. |
| `RECALCUL_INDICATEURS` | Refresh manuel de la vue matérialisée `mv_indicateurs_budget` via `POST /api/v1/budget/indicateurs/refresh` (Lot 3.6). Permission `BUDGET.LIRE` (tout consultant peut rafraîchir). | success ou failure | `entiteCible = 'mv_indicateurs_budget'`, `idCible = NULL`, `payloadApres = { dureeMs, nbLignes }`, `dureeMs` consigné. |
| `IMPORT_BUDGET_BULK` | Import en masse de saisie budgétaire via `POST /api/v1/budget/import` (Lot 3.7) — fichier CSV/XLSX. Une **seule** entrée par appel, succès **ou** rollback (>10 % erreurs). Permission `BUDGET.SAISIR`. | success ou failure | `entiteCible = 'fait_budget'`, `idCible = "version=X/scenario=Y"`, `payloadApres = { fichier, formatDetecte, lignesTotal, lignesInserees, lignesModifiees, lignesIgnorees, lignesRejetees, transactionRollback, warningsCount }`, `dureeMs` consigné. |
